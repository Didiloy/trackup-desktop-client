...
const hasMetadata = computed(() => props.metadataDefinitions && props.metadataDefinitions.length > 0)
...
<template>
    <div class="rounded-3xl bg-surface-100 ring-1 ring-surface-200/60 p-5 shadow-sm h-full">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex flex-wrap gap-1 mt-1">
                        ...
                        <span
                            ...
                            class="text-[10px] px-1.5 py-0.5 rounded bg-surface-200 text-surface-600 font-medium"
                        >
                            {{ meta.choices.length }} {{ t('common.choices') }}
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div v-else class="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p class="text-sm text-surface-600 font-medium">{{ t('userInterface.serverActivitiesView.noMetadata') }}</p>
            <p class="text-xs text-surface-500 mt-1 max-w-[200px]">
                {{ t('userInterface.serverActivitiesView.noMetadataDescription') }}
            </p>
        </div>
    </div>
</template>
