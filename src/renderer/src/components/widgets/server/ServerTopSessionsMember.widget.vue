...
defineOptions({
    widgetMetadata: {
        id: 'server-top-sessions-member',
        title_key: 'widgets.server.top_sessions_member.title',
        icon: 'pi pi-calendar',
        description_key: 'widgets.server.top_sessions_member.description',
        category: {
...
const top_member_id = computed(() => server_stats_store.getDetails?.server_stats.top_sessions_member)
        ...
        <template #header>
            <div class="px-5 pt-5 pb-3">
                <div class="flex items-center gap-2">
                    <Icon icon="pi pi-calendar" class="text-green-500" />
                    <h3
                        class="text-lg font-bold text-surface-900"
                        v-tooltip.bottom="t('views.server_stats.top_sessions_member_tooltip')"
                    >
                        {{ t('views.server_stats.top_sessions_member') }}
                    </h3>
                </div>
            </div>
        </template>
        ...
        >
            ...
            <div class="text-center w-full">
                ...
                <p class="text-sm text-surface-500">
                    {{ t('views.server_stats.most_sessions_member') }}
                </p>
                <div
                    v-if="member_stats"
