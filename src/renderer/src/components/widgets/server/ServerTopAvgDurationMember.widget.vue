...
defineOptions({
    widgetMetadata: {
        id: 'server-top-avg-duration-member',
        title_key: 'widgets.server.top_avg_duration_member.title',
        icon: 'pi pi-clock',
        description_key: 'widgets.server.top_avg_duration_member.description',
        category: {
...
const top_member_id = computed(
    () => server_stats_store.getDetails?.server_stats.top_avg_duration_member
)
...
const avg_duration = computed(() => {
    if (!member_stats.value) return 0
    const sessions = member_stats.value.total_sessions ?? 0
    const duration = member_stats.value.total_duration ?? 0
    return sessions > 0 ? Math.round(duration / sessions) : 0
})
        ...
        <template #header>
            <div class="px-5 pt-5 pb-3">
                <div class="flex items-center gap-2">
                    <Icon icon="pi pi-clock" class="text-orange-500" />
                    <h3
                        class="text-lg font-bold text-surface-900"
                        v-tooltip.bottom="t('views.server_stats.top_avg_duration_member_tooltip')"
                    >
                        {{ t('views.server_stats.top_avg_duration_member') }}
                    </h3>
                </div>
            </div>
        </template>
        ...
        >
            ...
            <div class="text-center w-full">
                ...
                <p class="text-sm text-surface-500">
                    {{ t('views.server_stats.best_avg_duration_member') }}
                </p>
                <div
                    v-if="member_stats"
                ...
                >
                    <span class="flex items-center gap-1">
                        ...
                        {{ formatMinutesToLabel(avg_duration) }} moy.
                    </span>
                    <span class="flex items-center gap-1">
