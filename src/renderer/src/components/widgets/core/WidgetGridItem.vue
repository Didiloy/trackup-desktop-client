...
<template>
    <div class="widget-wrapper h-full w-full relative">
        ...
        <template v-if="isEditing">
            ...
            <div
                class="absolute inset-0 border-2 border-dashed border-primary-200 rounded-xl pointer-events-none z-10"
            />

            <!-- Header bar (with pointer events for buttons) -->
            <div
                class="absolute top-0 left-0 right-0 flex items-center justify-between px-3 py-1.5 bg-primary-500/20 backdrop-blur-md rounded-t-xl z-20"
            >
                <div class="widget-drag-handle flex-1 cursor-move flex items-center gap-2">
                    <i class="pi pi-arrows-alt text-white text-xs" />
            ...
            <div class="absolute inset-x-0 top-8 bottom-3 cursor-move z-10" />
        </template>
    </div>
</template>
