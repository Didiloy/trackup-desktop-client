...
<template>
    <BaseWidgetContainer :title="t('widgets.member.details_summary.title')" :loading="isLoading">
        ...
        <div v-if="details" class="space-y-6">
            ...
            <!-- Favorite Activity -->
            <div
                v-if="favoriteActivity"
                class="p-4 rounded-2xl bg-primary-50 border border-primary-100"
            >
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div
                            class="text-[10px] text-primary-600 uppercase font-bold tracking-wider mb-0.5"
                        >
                        ...
                        <div class="text-xs text-surface-500">
                            {{
                                t('views.server_members.details.activity_sessions', {
                                    count: favoriteActivity.sessions_count
                                })
                            }}
                        </div>
                    </div>
            ...
            <div v-if="details.patterns" class="space-y-3">
                ...
                <div
                    class="flex items-center justify-between p-3 rounded-xl bg-surface-50 border border-surface-100"
                >
                    <div class="flex items-center gap-2">
                        <i class="pi pi-bolt text-amber-500"></i>
                        <span class="text-sm">{{
                            t('views.server_members.details.current_streak')
                        }}</span>
                    </div>
                    <span class="font-bold"
                        >{{ details.patterns.streak_current }} {{ t('common.time.days') }}</span
                    >
                </div>
            </div>
        </div>
