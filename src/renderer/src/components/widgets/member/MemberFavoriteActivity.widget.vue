...
defineOptions({
    widgetMetadata: {
        id: 'member-favorite-activity',
        title_key: 'widgets.member.favorite_activity.title',
        icon: 'pi pi-star-fill',
        description_key: 'widgets.member.favorite_activity.description',
        category: {
...
const subtitle = computed(() =>
    statsData.value?.favorite_activity
        ? t('views.server_members.details.activity_sessions', {
              count: statsData.value.favorite_activity.sessions_count
          })
        : ''
)
</script>
...
<template>
    <BaseOverviewStatWidget
        :label="t('views.server_members.details.favorite_activity')"
        :value="value"
        :sub-value="subtitle"
        icon="pi pi-star-fill"
